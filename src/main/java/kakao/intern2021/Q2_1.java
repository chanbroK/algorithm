...
//        String[][] places = {{"PXXXX", "XPXXX", "XXXPP", "XXXXX", "XXXXX"}};
    ...
    static class Solution {
        public int checkPlace(String[][] place, int row, int col) {
            ...
            if (row - 2 >= 0 && place[row - 2][col].equals("P") && place[row - 1][col].equals("O")) {
                // 상2
                return 0;
            }
            if (row + 2 < place.length && place[row + 2][col].equals("P") && place[row + 1][col].equals("O")) {
                // 하2
                return 0;
            }
            if (col - 2 >= 0 && place[row][col - 2].equals("P") && place[row][col - 1].equals("O")) {
                // 좌2
                return 0;
            }
            if (col + 2 < place.length && place[row][col + 2].equals("P") && place[row][col + 1].equals("O")) {
                // 우2
                return 0;
            }
            // 대각선
            if ((row - 1 >= 0 && col - 1 >= 0 && place[row - 1][col - 1].equals("P")) && (place[row - 1][col].equals("O") || place[row][col - 1].equals("O"))) {
                // 좌상
                return 0;
            }
            if (row + 1 < place.length && col - 1 >= 0 && place[row + 1][col - 1].equals("P") && (place[row + 1][col].equals("O") || place[row][col - 1].equals("O"))) {
                // 좌하
                return 0;
            }

            if (row - 1 >= 0 && col + 1 < place.length && place[row - 1][col + 1].equals("P") && (place[row - 1][col].equals("O") || place[row][col + 1].equals("O"))) {
                // 우상
                return 0;
            }

            if (row + 1 < place.length && col + 1 < place.length && place[row + 1][col + 1].equals("P") && (place[row + 1][col].equals("O") || place[row][col + 1].equals("O"))) {
                // 우하
                return 0;
            }
            return 1;
